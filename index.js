...
let log = "" /// log channel id
let prefix = "" /// prefix bot 
let modroles = "" //// mod role
let botrole = "" /// bot role
let ticketCategory = "" //// category channel id
...
client.on("ready", () => {
  ...
  client.user.setActivity('DM to contact Staff || !help')
})
...
client.on("message", async message => {
  ...
  if(message.channel.type === "dm"){
    ...
    if(active === null){
      ...
      channel.setTopic(`#${actualticket} (Open) | ${prefix}complete to close this ticket | Modmail for ${message.author.username}`)
  ...
  if(support){
    ...
    if(message.content.toLowerCase().startsWith(`${prefix}reply`)){
    ...
    if(message.content.toLowerCase().startsWith(`${prefix}areply`)){
      var isPause = await table.get(`suspended${support.targetID}`);
      let isBlock = await table.get(`isBlocked${support.targetID}`);
      if(isPause === true) return message.channel.send("This ticket already paused. Unpause it to continue.")
      if(isBlock === true) return message.channel.send("The user is blocked. Unblock them to continue or close the ticket.")
    ...
    if(message.content.toLowerCase() === `${prefix}finish`){
    ...
    if(message.content.toLowerCase() === `${prefix}pause`){
      ...
      .setDescription(`⏸️ This thread has been **locked** and **suspended**. Do \`${prefix}continue\` to cancel.`)
      .setTimestamp()
      .setColor("YELLOW")
    ...
    if(message.content.toLowerCase() === `${prefix}continue`){
      var isPause = await table.get(`suspended${support.targetID}`);
...
client.on('message', async message => {
  if(message.content.toLowerCase().startsWith(`${prefix}khelp`)){
        var embedd = new Discord.MessageEmbed()
          .setTitle(`Menu`)
          .setDescription(`My prefix is ${prefix}`)
          .addField(`${prefix}reply`, `to reply a thread.`, false)
          .addField(`${prefix}areply`, `to reply a thread anonymously.`, false)
          .addField(`${prefix}block`, `to block user from dming bot.`, false)
          .addField(`${prefix}pause`, `to stop receiving messagesfrom user.`, false)
          .addField(`${prefix}continue`, `to continue the thread.`, false)
          .addField(`${prefix}finish`, `to send a custom finished greetings.`, false)
          .addField(`${prefix}complete`, `to close the thread.`, false)
          .setColor("YELLOW")
          .setTimestamp()
